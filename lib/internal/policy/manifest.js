...
class Manifest {
   ...
   *
   * Used to change the canonical location which dependencies are
   * loaded from. This affects things like import.meta.url
   */
  #resourceBaseURLs = new SafeMap();
  /**
  ...
  constructor(obj, manifestHREF) {
    ...
    for (let i = 0; i < jsonResourcesEntries.length; i++) {
      ...
      const { cascade, dependencies, integrity, baseURL } = descriptor;
      ...
      if (typeof baseURL === 'string') {
        const resolvedBaseURL = resolve(baseURL, manifestHREF).href;
        this.#resourceBaseURLs.set(href, resolvedBaseURL);
      }
   ...
   */
  getBaseURL(href) {
    if (this.#resourceBaseURLs.has(href)) {
      return this.#resourceBaseURLs.get(href);
    }
    return href;
  }
  ...
  getDependencyMapper(requester) {
    ...
    const baseURLs = this.#resourceBaseURLs;
    /**
    ...
    const baseURL = baseURLs.has(requesterHREF) ?
      baseURLs.get(requesterHREF) :
      requesterHREF;
    return {
